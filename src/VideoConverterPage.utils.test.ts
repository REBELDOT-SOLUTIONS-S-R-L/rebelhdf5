...
function requireImageData(value: ImageData | null): ImageData {
  if (!value) {
    throw new Error('Expected the renderer to produce ImageData.');
  }
  return value;
}
...
describe('createFrameRenderer', () => {
  it('copies RGBA source data directly', () => {
    ...
    const view = createFrameRenderer(context, 1, 1, 4);
    const frames = new Uint8Array([10, 20, 30, 40]);
    view(frames, 0);
    expect([...requireImageData(lastImageData()).data]).toEqual([
      10, 20, 30, 40,
    ]);
  });
  ...
  it('expands RGB source data with an opaque alpha channel', () => {
    const { context, lastImageData } = fakeContext();
    const view = createFrameRenderer(context, 1, 1, 3);
    view(new Uint8Array([10, 20, 30]), 0);
    expect([...requireImageData(lastImageData()).data]).toEqual([
      10, 20, 30, 255,
    ]);
  });
  ...
  it('replicates a grayscale value across RGB with opaque alpha', () => {
    const { context, lastImageData } = fakeContext();
    const view = createFrameRenderer(context, 1, 1, 1);
    view(new Uint8Array([128]), 0);
    expect([...requireImageData(lastImageData()).data]).toEqual([
      128, 128, 128, 255,
    ]);
  });
  ...
  it('renders every pixel of a multi-pixel grayscale frame', () => {
    const { context, lastImageData } = fakeContext();
    const view = createFrameRenderer(context, 2, 1, 1);
    view(new Uint8Array([100, 200]), 0);
    expect([...requireImageData(lastImageData()).data]).toEqual([
