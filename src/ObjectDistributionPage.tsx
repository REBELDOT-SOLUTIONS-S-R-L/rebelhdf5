...
import { ObjectDistributionEmptyState } from './ObjectDistributionEmptyState';
...
const VIEW_TABS: { id: ObjectViewTab; label: string }[] = [
...
function isScatterTab(tab: ObjectViewTab): boolean {
  return tab === 'scatter';
}

function hasOpenedFiles(openedFileCount: number): boolean {
  return openedFileCount > 0;
}
...
function ObjectDistributionPage() {
  ...
  useEffect(() => {
    ...
    async function loadDistribution() {
      try {
        const nextResult = await loadObjectDistribution({
  ...
  function renderLoadStatus() {
    if (!loadError && !loading && !teleopMessage) {
      return null;
    }
    ...
    return (
      <section className={styles.messageCard}>
        <p className={styles.infoText}>
          {loading ? 'Loading object-distribution data…' : teleopMessage}
        </p>
      </section>
    );
  }

  function handlePlotClick(event: unknown) {
  ...
  function getPlotClickHandler() {
    return isScatterTab(activeTab) ? handlePlotClick : undefined;
  }

  function getPlotDoubleClickHandler() {
    return isScatterTab(activeTab) ? handlePlotDoubleClick : undefined;
  }
  ...
  function renderMissingAnalysis() {
    ...
    return <p className={styles.infoText}>{analysisMessage}</p>;
  }

  function renderEmptyRecommendations() {
    if (!analysisResult || analysisResult.recommendations.length > 0) {
      return null;
    }

    return (
      <p className={styles.infoText}>
        No connected high-risk regions met the current neighborhood-support
        threshold.
      </p>
    );
  }

  function renderTabSpecificControls() {
    if (isScatterTab(activeTab)) {
      return (
      ...
      <>
        ...
        <label className={styles.checkboxField} htmlFor="object-show-teleop">
          <input
            ...
            type="checkbox"
            aria-label="Show teleop overlay"
  ...
  function renderEmptyState() {
    if (hasOpenedFiles(opened.length)) {
      return null;
    }

    return <ObjectDistributionEmptyState openedFileCount={opened.length} />;
  }
        ...
        <header className={styles.header}>
          <div>
            ...
            <p className={styles.subtitle}>
              Compare generated datasets and map failure coverage over the
              object initial-pose reset space. Use the analysis tabs to find
          ...
          <>
            ...
            <section className={styles.chartCard}>
              <Plot
                ...
                data={plotData}
                layout={plotLayout}
                onClick={getPlotClickHandler()}
                onDoubleClick={getPlotDoubleClickHandler()}
                useResizeHandler
                style={{ width: '100%' }}
                ...
                <div className={styles.recommendationsHeader}>
                  <div>
                    ...
                    <p className={styles.recommendationsText}>
                      Ranked from connected high-risk neighborhoods in the
                      spatial slices, using failure confidence, local support,
                      and nearby teleop density.
                    </p>
                  </div>
                </div>
                    ...
                    <div className={styles.recommendationsGrid}>
                      {analysisResult.recommendations.map(
                        (recommendation, index) => (
                          <article
                            key={`${recommendation.sliceRowIndex}-${recommendation.sliceColIndex}-${recommendation.xStart}-${recommendation.yStart}`}
                          ...
                          >
                            ...
                            <p className={styles.recommendationRate}>
                              {formatPercent(recommendation.failureRate)} local
                              fail
                            </p>
                            ...
                            <p className={styles.recommendationLine}>
                              ...
                              total /{' '}
                              {formatSupport(
                                recommendation.smoothedFailedSupport,
                              )}{' '}
                              failed
                            </p>
                            <p className={styles.recommendationLine}>
