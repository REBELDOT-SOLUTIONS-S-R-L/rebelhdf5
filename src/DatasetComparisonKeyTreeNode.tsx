...
import { type KeyTreeNode } from './dataset-selection';
import styles from './DatasetComparisonPage.module.css';

interface DatasetComparisonKeyTreeNodeProps {
  node: KeyTreeNode;
  depth: number;
...
export function DatasetComparisonKeyTreeNode({
  node,
  depth,
  selectedKeys,
  collapsedGroups,
  ...
  onToggleGroup,
  onToggleCollapse,
}: DatasetComparisonKeyTreeNodeProps) {
  if (node.keyInfo) {
    return (
      <label
